#region LGPL License
/*----------------------------------------------------------------------------
* This file (CK.Plugin.Runner\Planner\ConfigurationSolver\ServiceData.StaticPropagation.cs) is part of CiviKey. 
*  
...
namespace CK.Plugin.Hosting
{
    partial class ServiceData
    {
        ...
        class CommonServiceReferences
        {
            ...
            {
                Debug.Assert( serviceRequirement >= RunningRequirement.MustExist );
                Ref r = _firstRef;
                while( r != null )
                {
            ...
            void Initialize( Dictionary<IServiceInfo, ServiceData> allServices, PluginData p )
            {
                ...
                {
                    _firstRef = new Ref( _firstRef, allServices[sr.Reference], sr.Requirements );
                }
            }
            ...
            void IntersectWith( PluginData p )
            {
                ...
                foreach( IServiceReferenceInfo sr in p.PluginInfo.ServiceReferences )
                {
                    ...
                    if( exist != null )
                    {
                        if( exist.Requirement > sr.Requirements ) exist.Requirement = sr.Requirements;
        ...
        void InitializePropagation( int nbAvailable, bool fromConfig )
        {
            ...
            else if( MinimalRunningRequirement >= RunningRequirement.MustExist )
            {
        ...
        {
            Debug.Assert( !Disabled && MinimalRunningRequirement >= RunningRequirement.MustExist && TotalAvailablePluginCount > 1 );
