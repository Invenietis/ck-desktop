...
namespace CK.Plugin.Hosting
{
    ...
    {
        readonly IActivityMonitor _log;
        ...
        public PluginHost( IActivityMonitor monitor = null)
            : this( CatchExceptionGeneration.HonorIgnoreExceptionAttribute, monitor )
        {
        }

        internal PluginHost( CatchExceptionGeneration catchMode, IActivityMonitor monitor = null )
        {
            _log = monitor ?? new ActivityMonitor( "Plugin Host" );
        ...
        {
            using( _log.OpenInfo().Send( "Host Apply" ) )
            {
                ...
                foreach( PluginProxy p in toStop )
                {
                    if( p.Status > InternalRunningStatus.Stopped )
                    {
                        try
                        {
                            ...
                            p.RealPlugin.Stop();
                            _log.Info().Send( "The {0} plugin has been successfully stopped.", p.PublicName );
                        }
                        catch( Exception ex )
                        {
                            _log.Error().Send( ex, "There has been a problem when stopping the {0} plugin.", p.PublicName );
                ...
                foreach( PluginProxy p in toStop )
                {
                    try
                    {
                        if( p.Status > InternalRunningStatus.Stopped )
                        {
                            ...
                            _log.Info().Send( "The {0} plugin has been successfully torn down.", p.PublicName );
                        }
                    }
                    catch( Exception ex )
                    {
                        _log.Error().Send( ex, "There has been a problem when tearing down the {0} plugin.", p.PublicName );
                ...
                foreach( PluginProxy p in toDisable )
                {
                    ...
                    {
                        _log.Error().Send( ex, "There has been a problem when disposing the {0} plugin.", p.PublicName );
                ...
                for( int i = 0; i < toStart.Count; i++ )
                {
                    ...
                    try
                    {
                        ...
                        info.Clear();
                        _log.Info().Send( "The {0} plugin has been successfully set up.", p.PublicName );
                    }
                    catch( Exception ex )
                    {
                        _log.Error().Send( "There has been a problem when setting up the {0} plugin.", p.PublicName );
                ...
                for( int i = 0; i < toStart.Count; i++ )
                {
                    ...
                    try
                    {
                        ...
                        p.RealPlugin.Start();
                        _log.Info().Send( "The {0} plugin has been successfully started.", p.PublicName );
                    }
                    catch( Exception ex )
                    {
                        // Emitted as low level log.
                        _log.Error().Send( ex, "There has been a problem when starting the {0} plugin.", p.PublicName );
